...
void test_random_ips(struct tbm_trie *trie)
{
...
int del_routes(struct tbm_trie *trie, FILE *fp)
{
    ...
    while((read = getline(&line, &len, fp)) != -1){
        if (i & 0x01) {
            ...
            void *a = tbm_search(trie, org_ip);
            key = (uint32_t)(uint64_t)a;
            if (key != 0) {
                //printf("line %s, cidr %d key %d, org_ip %x\n", buf, cidr, key,org_ip );
            }
            if (tbm_prefix_exist(trie, ip, cidr)){
...
int load_routes(struct tbm_trie *root, FILE *fp)
{
    ...
    while((read = getline(&line, &len, fp)) != -1){
        if (i & 0x01) {
            ...
            //    printf("here\n");
            //}
            tbm_insert_prefix(root, ip, cidr,(void*)(key));
            //hash_trie_insert(ip,cidr,(struct next_hop_info*)(key));
